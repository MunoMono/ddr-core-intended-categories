...
export default function BarChart({ data = [], title, color = "#276FBF", horizontal = false }) {
  const ref = useRef();
  ...
  useEffect(() => {
    ...
    const xDomain = horizontal ? [0, d3.max(sorted, d => d[1])] : sorted.map(d => d[0]);
    const yDomain = horizontal ? sorted.map(d => d[0]) : [0, d3.max(sorted, d => d[1])];
    ...
    setLegendItems(sorted.slice(0, 8).map(([label]) => ({ label, color: colorScale(label) })));
    ...
    const bars = barGroup.selectAll("rect")
      ...
      .on("mouseover", (event, d) => {
        ...
        tooltip.html(`<strong>${d[0]}</strong><br/>${d[1]} staff`)
    ...
    // ✅ Fixed transition — prevents negative width/height
    bars.transition()
      .delay((_, i) => i * 40)
      .duration(900)
      .ease(d3.easeCubicOut)
      .attr("width", d => (horizontal ? Math.max(0, x(d[1]) - x(0)) : x.bandwidth()))
      .attr("height", d => (horizontal ? y.bandwidth() : Math.max(0, y(0) - y(d[1]))))
    ...
    svg.append("text")
      ...
      .attr("x", width - 220)
      .attr("y", horizontal ? margin.t + 10 : y(maxValue) - 10)
      .text(`Most common: ${maxLabel} (${maxValue})`);
    ...
    <div>
      ...
      {data.length ? (
        <p className="chart-insight">
          The <strong>{topRole}</strong> role dominates with <strong>{topValue}</strong> staff,
          indicating a concentration of departmental expertise.
        </p>
      ) : (
